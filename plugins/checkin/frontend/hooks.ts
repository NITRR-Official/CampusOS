...
export function useCreateCheckIn() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['checkins', 'event', variables.eventId] });
      queryClient.invalidateQueries({ queryKey: ['checkins', 'status', variables.eventId, variables.userId] });
      queryClient.invalidateQueries({ queryKey: ['checkins', 'stats', variables.eventId] });
    },
  });
}
...
export function useScanQRCode() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['checkins', 'event', data.eventId] });
      queryClient.invalidateQueries({ queryKey: ['checkins', 'status', data.eventId, data.userId] });
      queryClient.invalidateQueries({ queryKey: ['checkins', 'stats', data.eventId] });
    },
  });
}

...
export function RoleEditorSheet({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (role) {
        await updateRole(clubId, role._id, payload);
        toast({ title: 'Success', description: 'Role updated successfully' });
      } else {
        // Provide a default high hierarchy level for new roles, backend or RolesManager handles ordering later
        await createRole(clubId, {
          ...payload,
          hierarchyLevel: 0,
          roleType: 'role'
        });
